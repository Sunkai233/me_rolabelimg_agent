...
class LabelFile(object):
    ...
    def savePascalVocFormat(self, filename, shapes, imagePath, imageData,
        ...
        for shape in shapes:
            ...
            # 新增：检查是否为直线模式
            isLine = shape.get('isLine', False)

            # if shape is normal box, save as bounding box
            if not isRotated and not isLine:
                bndbox = LabelFile.convertPoints2BndBox(points)
                writer.addBndBox(bndbox[0], bndbox[1], bndbox[2],
                                 bndbox[3], label, difficult)
            else:  # if shape is rotated box or line, save as rotated bounding box
                # 新增：如果是直线模式，使用预先计算好的参数
                if isLine and 'cx' in shape:
                    # 直线模式：直接使用传递过来的旋转矩形参数
                    cx = shape['cx']
                    cy = shape['cy']
                    w = shape['w']
                    h = shape['h']
                    angle = shape['angle']
                    writer.addRotatedBndBox(cx, cy, w, h, angle, label, difficult)
                else:
                    # 普通旋转矩形：使用原有逻辑计算
    ...
    def convertPoints2RotatedBndBox(shape):
        ...
        # 新增：如果是直线模式（只有2个点），使用不同的计算方式
        if len(points) == 2:
            # 直线模式：计算长度作为宽度
            w = math.sqrt((points[1][0] - points[0][0]) ** 2 +
