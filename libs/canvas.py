...
class Canvas(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 新增：直线模式标识
        self.canDrawLine = False  # 是否启用直线模式
    ...
    def mouseMoveEvent(self, ev):
        ...
        if self.drawing():
            ...
            if self.current:
                ...
                elif len(self.current) > 1 and self.closeEnough(pos, self.current[0]) and not self.current.isLine:
                    # 直线模式不需要闭合到起点
                ...
                # 状态栏显示
                if self.current.isLine:
                    # 直线模式：显示长度和角度
                    dx = pos.x() - self.line[0].x()
                    dy = pos.y() - self.line[0].y()
                    length = math.sqrt(dx * dx + dy * dy)
                    angle = math.atan2(dy, dx) * 180 / math.pi
                    self.status.emit("Length: %.1f, Angle: %.1f°" % (length, angle))
                else:
                    # 旋转矩形模式：显示宽高
                    self.status.emit(
                        "width is %d, height is %d." % (pos.x() - self.line[0].x(), pos.y() - self.line[0].y()))
            return
        ...
        if Qt.RightButton & ev.buttons():
            if self.selectedVertex() and self.selectedShape.isRotated:
                ...
                self.repaint()
            # 直线模式：右键拖动旋转直线
            elif self.selectedVertex() and self.selectedShape.isLine:
                self.boundedRotateLine(pos)
                self.shapeMoved.emit()
                self.repaint()
        ...
        if Qt.LeftButton & ev.buttons():
            if self.selectedVertex():
                if self.selectedShape.isLine:
                    # 直线模式：移动端点
                    self.boundedMoveLineVertex(pos)
                else:
                    # 旋转矩形模式：移动顶点
    ...
    def handleDrawing(self, pos):
        if self.current and self.current.reachMaxPoints() is False:
            ...
            self.current = Shape()
            self.current.isLine = self.canDrawLine  # 设置是否为直线模式
            self.current.addPoint(pos)
    ...
    # 新增：直线模式的顶点移动
    def boundedMoveLineVertex(self, pos):
        """移动直线的端点"""
        index, shape = self.hVertex, self.hShape
        point = shape[index]
        ...
        if len(shape.points) == 2:
            shape.center = QPointF(
                (shape.points[0].x() + shape.points[1].x()) / 2,
                (shape.points[0].y() + shape.points[1].y()) / 2
            )
            dx = shape.points[1].x() - shape.points[0].x()
            dy = shape.points[1].y() - shape.points[0].y()
            shape.direction = math.atan2(dy, dx)
            length = math.sqrt(dx * dx + dy * dy)
            self.status.emit("Length: %.1f" % length)

    # 新增：直线模式的旋转
    def boundedRotateLine(self, pos):
        """旋转直线"""
        index, shape = self.hVertex, self.hShape
        if len(shape.points) != 2:
            return
        ...
        for i, p in enumerate(shape.points):
            shape.points[i] = shape.rotatePoint(p, angle)

        shape.direction -= angle
        shape.direction = shape.direction % (2 * math.pi)
        self.prevPoint = pos
    ...
    def paintEvent(self, event):
        ...
        for shape in self.shapes:
            if (shape.selected or not self._hideBackround) and self.isVisible(shape):
                ...
                if shape.isLine:
                    ...
                    shape.paint(p)
                elif (shape.isRotated and not self.hideRotated) or (not shape.isRotated and not self.hideNormal):
                    shape.fill = shape.selected or shape == self.hShape
                    shape.paint(p)
        ...
        # Paint rect (for non-line mode)
        if self.current is not None and len(self.line) == 2 and not self.current.isLine:
            leftTop = self.line[0]
    ...
    def finalise(self):
        assert self.current
        if not self.current.isLine:
            # 旋转矩形模式
            self.current.isRotated = self.canDrawRotatedRect
        # 直线模式：isLine 已经在 handleDrawing 中设置
        self.current.close()
    ...
    def keyPressEvent(self, ev):
        ...
        if key == Qt.Key_Escape and self.current:
            ...
            self.moveOnePixel('Down')
                ...
                (self.selectedShape.isRotated or self.selectedShape.isLine) and not self.rotateOutOfBound(0.1):
            self.selectedShape.rotate(0.1)
            self.shapeMoved.emit()
            self.update()
        elif key == Qt.Key_X and self.selectedShape and \
                (self.selectedShape.isRotated or self.selectedShape.isLine) and not self.rotateOutOfBound(0.01):
