...
# 导入新增模块
from result_viewer import ResultViewerDock
from config_dialog import ModelConfigDialog
from xml_to_json_converter import convert_single_xml_to_json
from meter_predictor import get_predictor
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        # 创建结果查看器dock
        self.resultViewerDock = ResultViewerDock(self)
        self.addDockWidget(Qt.LeftDockWidgetArea, self.resultViewerDock)
        self.resultViewerDock.setMinimumHeight(250)
        self.resultViewerDock.swapRequested.connect(self.swapImages)
        ...
        openResultDir = action('&Open Result Dir', self.openResultDir,
                               'Ctrl+Shift+R', 'open', u'Open Result Directory')
        ...
        # 添加预测相关动作
        predict = action('&Predict Meter', self.predictMeter,
                         'Ctrl+P', 'verify', u'Predict meter reading')

        modelConfig = action('&Model Config', self.openModelConfig,
                             'Ctrl+M', 'save', u'Configure prediction models')
        ...
        createLine = action('Create\nLine', self.createLineShape,
                            'l', 'newLine', u'Draw a new Line (saved as thin rotated box)', enabled=False)
        ...
        resultViewer = self.resultViewerDock.toggleViewAction()
        resultViewer.setText('Show/Hide Result Viewer')
        resultViewer.setShortcut('Ctrl+Shift+P')
        ...
        self.actions = struct(save=save, saveAs=saveAs, open=open, close=close,
                              ...
                              create=create, createRo=createRo, createLine=createLine, delete=delete, edit=edit,
                              copy=copy,
                              ...
                              fileMenuActions=(
                                  open, opendir, openResultDir, save, saveAs, close, None,
                                  modelConfig, predict, None, quit),
                              beginner=(), advanced=(),
        ...
        addActions(self.menus.file,
                   (open, opendir, openResultDir, changeSavedir, openAnnotation, self.menus.recentFiles, save, saveAs,
                    close, None,
                    modelConfig, predict, None, quit))
        addActions(self.menus.help, (help,))
        addActions(self.menus.view, (
            labels, resultViewer, advancedMode, None,
        ...
        self.actions.beginner = (
            open, opendir, openNextImg, openPrevImg, verify, save, None, create, createRo, createLine, copy, delete,
            None,
    ...
    def enableCreateLine(self, b):
        self.isEnableCreateLine = not b
        self.actions.createLine.setEnabled(self.isEnableCreateLine)
    ...
    def createLineShape(self):
        assert self.beginner()
        self.canvas.setEditing(False)
        self.canvas.canDrawRotatedRect = False
        self.canvas.canDrawLine = True
        self.actions.create.setEnabled(False)
        self.actions.createRo.setEnabled(False)
        self.actions.createLine.setEnabled(False)
    ...
    def loadFile(self, filePath=None):
        ...
        if self.isSwapped:
            self.isSwapped = False
            self.resultViewerDock.setWindowTitle("预测结果查看器")
    ...
    def loadResultImage(self):
        """加载当前图片对应的结果图片"""
        if not self.filePath or not self.resultDirPath:
            self.resultViewerDock.clear()
            return

        img_basename = os.path.basename(self.filePath)
        img_name, img_ext = os.path.splitext(img_basename)

        result_filename = img_name + '_result' + img_ext
        result_path = os.path.join(self.resultDirPath, result_filename)

        if not os.path.exists(result_path):
            for ext in ['.jpg', '.jpeg', '.png', '.bmp', '.JPG', '.JPEG', '.PNG', '.BMP']:
                result_filename = img_name + '_result' + ext
                result_path = os.path.join(self.resultDirPath, result_filename)
                if os.path.exists(result_path):
                    break

        if os.path.exists(result_path):
            success = self.resultViewerDock.load_result_image(result_path)
            if success:
                self.statusBar().showMessage('Loaded result image: %s' % os.path.basename(result_path), 2000)
        else:
            self.resultViewerDock.clear()
            self.statusBar().showMessage('Result image not found for: %s' % img_basename, 2000)

    def swapImages(self):
        """交换主界面和结果查看器的图片"""
        if not self.filePath or not self.resultDirPath:
            self.statusBar().showMessage('请先打开图片文件夹和结果文件夹', 3000)
            return

        if not self.isSwapped:
            current_main_path = self.filePath
            img_basename = os.path.basename(current_main_path)
            img_name, img_ext = os.path.splitext(img_basename)

            result_filename = img_name + '_result' + img_ext
            current_result_path = os.path.join(self.resultDirPath, result_filename)
            ...
            if not os.path.exists(current_result_path):
                self.statusBar().showMessage('未找到对应的结果图片', 3000)
                return

            self.originalImagePath = current_main_path
            ...
            self.imageData = read(current_result_path, None)
            image = QImage.fromData(self.imageData)
            if image.isNull():
                self.statusBar().showMessage('结果图片加载失败', 3000)
                return

            self.image = image
            self.filePath = current_result_path
            self.canvas.loadPixmap(QPixmap.fromImage(image))

            self.setFitWindow(True)

            self.resultViewerDock.load_result_image(self.originalImagePath)
            self.resultViewerDock.viewer.fit_to_window()

            self.isSwapped = True
            self.resultViewerDock.setWindowTitle("原始标注图片")
            self.statusBar().showMessage('已交换：主界面显示结果图片，左侧显示原始图片', 3000)

        else:
            if self.originalImagePath and os.path.exists(self.originalImagePath):
                self.loadFile(self.originalImagePath)

                self.isSwapped = False
                self.originalImagePath = None
                self.resultViewerDock.setWindowTitle("预测结果查看器")
                self.statusBar().showMessage('已恢复：主界面显示原始图片，左侧显示结果图片', 3000)
            else:
                self.statusBar().showMessage('无法恢复：找不到原始图片', 3000)

    def loadSwappedImage(self, originalImagePath):
        """在交换状态下加载图片"""
        if not os.path.exists(originalImagePath):
            self.statusBar().showMessage('图片不存在', 3000)
            return False

        img_basename = os.path.basename(originalImagePath)
        img_name, img_ext = os.path.splitext(img_basename)
        ...
        if not os.path.exists(result_path):
            self.statusBar().showMessage('未找到结果图片: %s' % result_filename, 3000)
            self.isSwapped = False
            self.originalImagePath = None
            self.resultViewerDock.setWindowTitle("预测结果查看器")
            self.loadFile(originalImagePath)
            return False
        ...
        if self.fileListWidget.count() > 0:
            try:
                index = self.mImgList.index(originalImagePath)
                fileWidgetItem = self.fileListWidget.item(index)
                fileWidgetItem.setSelected(True)
            except (ValueError, AttributeError):
                pass

        self.imageData = read(result_path, None)
        image = QImage.fromData(self.imageData)
        if image.isNull():
            self.statusBar().showMessage('结果图片加载失败', 3000)
            return False
        ...
        self.resultViewerDock.load_result_image(originalImagePath)
        self.resultViewerDock.viewer.fit_to_window()

        self.setWindowTitle(__appname__ + ' ' + result_path + ' (交换模式)')
        self.statusBar().showMessage('已加载: %s (交换模式)' % img_basename, 2000)
    ...
    def openResultDir(self, _value=False):
        """打开结果文件夹"""
        path = os.path.dirname(self.filePath) if self.filePath else '.'

        if self.resultDirPath is not None and len(self.resultDirPath) > 1:
            path = self.resultDirPath

        dirpath = ustr(QFileDialog.getExistingDirectory(self,
                                                        '%s - Open Result Directory' % __appname__, path,
                                                        QFileDialog.ShowDirsOnly
                                                        | QFileDialog.DontResolveSymlinks))

        if dirpath is not None and len(dirpath) > 1:
            self.resultDirPath = dirpath
            self.statusBar().showMessage('Result directory set to %s' % self.resultDirPath)
            self.statusBar().show()
    ...
    def openPrevImg(self, _value=False):
        ...
        if self.isSwapped:
            currentPath = self.originalImagePath
        else:
        ...
        try:
            currIndex = self.mImgList.index(currentPath)
            if currIndex - 1 >= 0:
                filename = self.mImgList[currIndex - 1]

                if filename:
                    if self.isSwapped:
                        self.loadSwappedImage(filename)
                    else:
                        self.loadFile(filename)
        except ValueError:
            self.statusBar().showMessage('无法切换图片：当前图片不在列表中', 3000)
            return
    ...
    def openNextImg(self, _value=False):
        ...
        if currentPath is None:
            ...
            try:
                ...
                filename = self.mImgList[0] if len(self.mImgList) > 0 else None
    ...
    def openModelConfig(self, _value=False):
        """打开模型配置对话框"""
        dialog = ModelConfigDialog(self)
        if dialog.exec_():
            self.statusBar().showMessage('模型配置已更新', 3000)

    def predictMeter(self, _value=False):
        """预测表计读数"""
        if not self.filePath:
            QMessageBox.warning(self, '警告', '请先打开一张图片！')
            return

        xml_path = os.path.splitext(self.filePath)[0] + '.xml'
        if not os.path.exists(xml_path):
            QMessageBox.warning(self, '警告', f'未找到标注文件：{xml_path}')
            return

        progress = QProgressDialog("正在预测...", "取消", 0, 100, self)
        progress.setWindowModality(Qt.WindowModal)
        progress.setWindowTitle("表计预测")
        progress.setValue(10)

        try:
            progress.setLabelText("转换XML到JSON...")
            progress.setValue(20)

            json_data = convert_single_xml_to_json(xml_path)
            if json_data is None:
                raise Exception("XML转JSON失败")

            progress.setLabelText("加载模型配置...")
            ...
            for key in ['zhizhen_file', 'youwei_file', 'dial_detection_model']:
                if not config.get(key) or not os.path.exists(config[key]):
                    QMessageBox.warning(self, '警告',
                                        f'模型文件不存在：{key}\n'
                                        f'路径：{config.get(key, "未配置")}\n\n'
                                        '请重新配置模型路径！')
                    progress.close()
                    return

            progress.setLabelText("初始化模型（首次加载较慢）...")
            progress.setValue(40)

            predictor = get_predictor()
            if not predictor.models_loaded:
                success, message = predictor.initialize_models(config)
                if not success:
                    raise Exception(message)

            progress.setLabelText("正在预测...")
            progress.setValue(60)

            success, result_path, message = predictor.predict(self.filePath, json_data)

            if not success:
                raise Exception(message)

            progress.setLabelText("加载结果...")
            progress.setValue(80)

            if result_path and os.path.exists(result_path):
                self.resultViewerDock.load_result_image(result_path)
                self.resultViewerDock.viewer.fit_to_window()
                self.resultViewerDock.setWindowTitle("预测结果")

                if not self.resultViewerDock.isVisible():
                    self.resultViewerDock.show()

                progress.setValue(100)
                progress.close()

                QMessageBox.information(self, '预测完成', message)
                self.statusBar().showMessage(f'预测完成: {message}', 5000)
            else:
                raise Exception("未生成结果图片")

        except Exception as e:
            progress.close()
            QMessageBox.critical(self, '错误', f'预测失败：{str(e)}')
            self.statusBar().showMessage(f'预测失败', 3000)
